...
function renderPlayerAdvancedSections(profile) {
  ...
  if (p.avatar_url || p.language || p.office || p.shield_endtime || p.burn_endtime) {
    html += '<section class="profile-section"><h2>プロフィール詳細</h2>';
    if (p.avatar_url) {
      const avatarUrl = normalizeProfileAssetUrl(p.avatar_url);
      html += '<div class="profile-identity"><img class="profile-avatar" src="' + esc(avatarUrl) + '" alt="" loading="lazy"><div><span>プレイヤーアイコン</span><b>取得済み</b></div></div>';
    }
