...
function renderPlayerShell(message, governorId, player = null, payload = null, notice = null, profile = null) {
  ...
  const content = player ? `
    <div class="hero"><div class="player-identity-main">${p.avatar_url ? '<img class="player-avatar-main" src="' + esc(normalizeProfileAssetUrl(p.avatar_url)) + '" alt="" loading="lazy">': '<span class="player-avatar-main player-avatar-empty">?</span>'}<div><div class="eyebrow">PLAYER PROFILE</div><h1>${esc(p.nick_name || "Unknown Player")}</h1><div class="sub">領主ID ${esc(p.governor_id)}</div></div></div><div class="kid">王国 ${esc(p.kid ?? "-")}</div></div>
    <div class="grid">
...
function renderPlayerAdvancedSections(profile) {
  ...
  if (p.language || p.office || p.shield_endtime || p.burn_endtime) {
